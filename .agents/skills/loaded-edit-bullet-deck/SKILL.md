...
Whenever a `BulletData` asset is added, moved, or removed, keep every shop pool
and the MainMenu bullet dictionary synchronized. Editor import automation may
perform this authoring-time update when no loaded scene has unsaved changes;
otherwise run `Tools > LOADED > Sync All Bullet Pools` after saving scenes.
Preserve existing catalog order, reject nulls and duplicates, and append new
assets deterministically. Do not add the full catalog to
`DeckManager.startingBullets`, which is gameplay starting inventory rather than
...
For every bullet addition, also run `BulletPoolIntegrityTests` and verify the
bullet appears in both shop offers and the MainMenu dictionary. Bullet
authoring is incomplete while any shop or dictionary catalog is missing the
asset.
