...
* 플레이어와 거리가 1칸보다 멀고 `Preferred Distance` 안이면 최대 수량까지 공격 스킬을 한 턴에 하나씩 추가 등록합니다.
...
적은 먼저 플레이어가 있는 방향을 확인하고 방향이 다르면 해당 턴에는 회전만 한다. Melee는 방향이 맞으면 거리와 관계없이 Queue를 생성하고 다음 적 턴에 첫 공격 스킬을 반드시 등록한다. 플레이어와 거리가 2칸 이상이면서 `Preferred Distance` 이내라면 이후 턴마다 공격 스킬을 최대 3개까지 추가한다. 등록할 공격 에셋이 하나뿐이면 같은 스킬을 반복하고, 여러 개면 `EnemyData.Actions`의 MeleeAttack 순서를 순환한다. Queue가 가득 찼거나 추가 등록 범위 밖이면 플레이어와 1칸 거리가 될 때까지 접근한다.
...
* 테스트 결과:
  ...
  * Melee 공격 행동은 `CreateQueue -> 첫 RegisterAttack -> 조건부 추가 RegisterAttack -> 거리 1칸 PrepareAttack -> 다음 턴 Fire` 순서로 진행된다.
  * Melee는 첫 스킬을 거리와 관계없이 등록하고, 거리 2 이상 및 Preferred Distance 이내에서만 추가 스킬을 등록한다.
...
22. 거리가 2칸 이상이면서 Preferred Distance 이내이면 최대 3개까지 한 턴에 하나씩 아이콘이 추가되는지 확인한다.
