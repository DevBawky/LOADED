...
- 탄환 삽입 시 실린더 UI가 짧게 눌렸다가 확대되며 복귀한다. 장전 회전은 약간 지나쳤다가 목표 각도에 정착한다.
- 실제 회전 완료 시 작은 잠금 반동과 기존 장전 클립의 낮은 볼륨 강조음을 재생한다. 최대 장전 수에 도달하면 낮은 피치·강한 반동·금빛 강조로 마무리한다.
- UI 회전과 반동은 unscaled time을 사용하되 일시정지 중에는 멈춘다. 연속 장전·UI 비활성화 시 이전 반동을 정리하며, 장전·턴 판정은 연출 완료를 기다리지 않는다.
