...
`EnemyDamageNumberDisplay.ShowAttackDamage()`는 `CombatImpactTier.Defeat`일 때도 `isCritical` 값을 확인하여 프리팹을 선택한다. 이후 처치 등급의 색상과 스케일을 적용하므로, 처치의 강조는 유지하면서 원래 공격의 일반·크리티컬 외형도 보존한다.

## 한 번의 연속 발사 기준 처치 콤보

콤보의 범위는 시간 기반의 연속 처치가 아니라 `PlayerShoot`의 한 번의 발사 시퀀스이다.
