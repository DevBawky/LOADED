...
HUD 콤보는 처치 시 8칸으로 갱신된다. 플레이어 행동 자체는 게이지를 소비하지 않으며, DUEL CLOCK이 100%에 도달하여 `WaveManager`가 적 행동 사이클 완료를 알릴 때마다 한 칸을 소비한다. 추가 처치가 발생한 카운트 자체는 기존처럼 소비하지 않으며, 처치 시 게이지는 다시 8칸으로 갱신된다.
