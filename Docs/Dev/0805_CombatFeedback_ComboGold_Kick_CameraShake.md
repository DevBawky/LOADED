...
처치 문구와 전반적인 처치 연출 강도는 모두 한 번의 실린더 발사 시퀀스 안에서 발생한 처치 수를 기준으로 한다. 기본값에서는 같은 발사 시퀀스의 처치 순서가 1 증가할 때마다 배율이 `0.2`씩 계속 증가한다. 이 배율은 처치 파티클 수와 크기, 방향 잔상, 화면 플래시, Volume Pulse, 슬로 모션, Fullscreen Impact, 처치 문구에 적용된다. 카메라 셰이크는 접근성 및 화면 안정성을 위해 연속 처치 수와 무관한 고정 처치 세기를 사용한다. 턴을 넘겨 유지되는 HUD `Combo`는 표시·통계·보너스 골드에만 사용하며 처치 연출 세기에는 관여하지 않는다.
