...
- 적 위치에는 약 `0.09`초 동안 국소 접촉 섬광, 관통 방향의 가는 빛과 불규칙 원호가 나타난다. 적 스프라이트 전체를 흰색으로 복제하지 않는다.
...
- 처치 위치에서 가장 넓은 탄도 아지랑이와 광학 산란, 강한 화면 탄성 복원과 방향성 화면 잔상, 처치용 Volume Pulse, 슬로 모션, 효과음과 처치 텍스트를 동시에 시작한다. 적 스프라이트 복제 잔상은 사용하지 않는다.
...
2. 어두운 적과 배경에서도 일반 공격의 국소 접촉 섬광, `Impact Snap`과 파열 헤일로가 분명히 보이고, 크리티컬, 최대 체력 75% 이상 피해, 처치 순서로 2차 헤일로·아지랑이 범위·화면 탄성·잔광·방향성 잔상 강도가 단계적으로 커지는지 확인한다.
3. 크리티컬 처치에서는 크리티컬 대미지 프리팹, 일반 처치에서는 일반 대미지 프리팹이 나오는지 확인한다.
4. 한 번의 연속 발사로 적 두 명 이상을 처치했을 때 `2연속 처치!`, `3연속 처치!`가 순서대로 표시되는지 확인한다.
5. 첫 처치에는 보너스 골드가 없고, 두 번째 처치부터 공식대로 보너스가 발생하는지 확인한다.
6. 마지막 두 적을 같은 발사로 처치했을 때 마지막 텍스트까지 콤보 수에 포함된 뒤 전투 완료가 발생하는지 확인한다.
7. 획득량만큼 골드 이미지가 생성되고, 각 이미지가 Money 패널에 도착할 때 수치가 1씩 증가하는지 확인한다.
8. 골드가 비행 중인 상태로 상점에 진입하면 남은 금액이 즉시 한 번만 반영되는지 확인한다.
9. 먼 거리로 밀어내도 발차기 이동이 `pushFlightDuration` 안에 끝나는지 확인한다.
10. 최대 체력의 25%, 50%, 100% 피해에서 카메라 셰이크 세기와 지속 시간이 선형 비율로 변하는지 확인한다.
11. 허공 발사에서 최대값의 40% 셰이크가 발생하는지 확인한다.
12. 짧은 셰이크와 연속 셰이크 뒤에도 카메라 위치가 점프하지 않고, 회전이 `(0, 0, 0)`으로 부드럽게 돌아오는지 확인한다.
