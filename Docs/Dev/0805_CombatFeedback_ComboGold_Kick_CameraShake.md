...
| 3 | 치명적인 피해 | 한 발의 실제 적용 피해가 대상 최대 체력의 75% 이상 | 크리티컬보다 우선하여 더 강한 Volume Pulse, 슬로 모션, 충격파·RGB Split·Radial Zoom·Directional Tear |
| 4 | 적 처치 | 해당 피해로 적의 체력이 0 이하가 됨 | 가장 강한 처치 연출, 처치 텍스트, 콤보 연출, 처치음, 보너스 골드 |

`CombatImpactTierUtility.DevastatingDamageRatio`는 `0.75f`이다. 한 공격이 크리티컬이면서 최대 체력의 75% 이상을 가했다면 `Devastating`이 `Critical`보다 우선한다. 적을 처치했다면 표현 등급은 `Defeat`가 된다.

## 공격 등급별 연출

### 발사 순간

- 기존 절차형 총구 섬광은 탄환색 외곽, 사격 방향의 주 화염과 회전된 보조 섬광을 사용한다. 같은 수명 동안 탄환 Primary Color에 가까운 짧은 Point `Light2D`가 주변의 Lit Sprite를 실제로 밝히며 크리티컬은 강도와 반경이 소폭 커진다.
- 동시에 화면 전체에는 완화된 총구 중심 당김, 사격 방향의 렌즈 이동과 약한 색 분리를 적용하고, 탄환 Primary Color의 희미한 전면 섬광을 짧게 더한다. 명중 연출과 구분되는 발사 전용 상태이므로 파열 헤일로는 만들지 않는다.
- 총구와 적 접촉 섬광은 같은 셰이더의 `Effect Mode`를 다르게 사용한다. 적 접촉 파동은 공통 화약색 대신 탄환 Primary Color를 유지하며, 모든 관련 Coroutine은 일시정지 중 비스케일 진행 시간을 누적하지 않는다.

### 일반 공격

- 모든 명중 등급의 국소 파형은 발사된 `BulletInstance`의 Primary Line Color와 업그레이드 레벨이 반영된 Secondary Line Color를 혼합한다. Secondary 비중을 더 높게 두되 검정 계열에서도 형태가 사라지지 않도록 소량의 백색 리프트를 적용하며, 탄환 정보가 없는 보조 처치 경로에서는 기존 Primary 계열 색으로 대체한다.
- 공격 위치를 중심으로 짧은 국소 충격 렌즈와 불규칙한 탄도 아지랑이를 표시한다. 압축파는 별도 색상 링을 그리지 않고 원본 화면의 UV만 굴절시키며, 일반 명중은 굴절 범위를 탄착점 주변에 제한한다.
- 첫 2~3프레임에는 밝은 배경 픽셀의 유무와 관계없이 보이는 강한 `Impact Snap`을 적용한다. 국소 화면을 순간적으로 압축하고 사격 방향으로 튕긴 뒤, 선명도와 대비를 올려 부드러운 아지랑이와 구분되는 단단한 타격점을 만든다.
- 압력파 경계에는 완전한 원 대신 3~5개의 끊어진 광학 원호로 이루어진 파열 헤일로를 표시한다. 진행 방향의 원호는 밝고 굵으며 반대편 틈은 짧게 어두워져 탄환 방향을 드러낸다.
- 적 위치에는 약 `0.09`초 동안 국소 접촉 섬광과 탄환 방향으로 튕기는 이중 스냅 마크, 관통 방향의 가는 빛과 불규칙 원호가 나타난다. 적 스프라이트 전체를 흰색으로 복제하지 않는다.
- 실제 탄환의 대표 색상은 작은 광학 잔광과 밝은 화면 픽셀의 산란에만 사용한다. 물리적인 탄흔, 굵은 파편, 잉크, 필름 문양은 표시하지 않는다.
- 적 스프라이트를 복제해 흰색으로 번쩍이고 확대하던 Hit Flash와 화면 전체 흰색 플래시는 사용하지 않는다.
- 후처리 셰이더에는 `Normal`, `Critical`, `Devastating`, `Defeat`를 0~3의 명시적인 등급으로 전달한다. 국소 잔광과 열기 먼지 애니메이션은 일시정지 중 진행되지 않는다.
...
- 일반 공격보다 Fullscreen Impact의 지속 시간과 강도가 증가하고, 열기 굴절이 사격 방향으로 더 넓게 이동한다.
- 국소 RGB 분리, 화면 탄성 변형, 첫 압력파 뒤의 2차 충격·보조 헤일로와 짧은 방향성 재샘플링 잔상을 추가한다. 적 접촉 섬광도 회전된 보조 원호 한 겹을 더 사용한다. 화면 전체 노출은 짧게 눌렸다 복원된다.
- 충격점에는 관통 방향의 가는 십자선과 중앙 코어가 나타나고, 주변의 약실 눈금이 빠르게 안쪽으로 잠긴다. 파티클 양보다 좁고 날카로운 형태를 강조해 치명적인 피해의 넓고 무거운 파열과 구분한다.
...
- 실제 적용 피해가 최대 체력의 75% 이상이면 발생한다.
- 크리티컬 여부보다 우선하여 `Devastating` 연출을 선택한다.
- 넓은 압축파와 강한 2차 충격, 탄도 아지랑이, 밝은 영역의 광학 Bloom, 화면 전체의 탄성 압축·복원과 방향성 잔상을 사용한다. 크리티컬보다 굴절 깊이와 잔광, 잔상 분리 거리가 커진다.
- 국소 파편은 먼저 충격점으로 수축한 뒤 바깥으로 파열되고, 잠시 뒤 끊어진 원호 형태의 2차 압력파가 재생된다. 이 2박자 연출은 고정 각도 패턴을 사용하므로 전투 RNG 순서를 소비하지 않는다.
...
- 처치 위치에서 가장 넓은 탄도 아지랑이와 광학 산란, 강한 화면 탄성 복원과 방향성 화면 잔상, 처치용 Volume Pulse, 슬로 모션, 효과음과 처치 텍스트를 동시에 시작한다.
- 적이 제거되기 전에 캡처한 스프라이트 스냅샷으로 짧은 명암 실루엣을 고정한 뒤, 탄환 방향으로 분리되는 2~6개의 잔상을 순차 재생한다. 이 복제본은 판정이나 적 수명에 관여하지 않는 일시적인 프레젠테이션 오브젝트이며 일시정지 중에는 진행하지 않는다.
- 마지막 적 처치는 실루엣 정지 시간을 늘리고, 여섯 약실이 충격점으로 잠긴 뒤 바깥으로 깨지는 전용 실린더 봉인 연출을 추가한다. 기존 슬로 모션과 Fullscreen Impact 지속 시간 강화도 유지한다.
- 한 번의 연속 발사 안에서 처치 수가 늘어나면 뒤의 처치일수록 연출 강도가 증가한다.
- 같은 프레임에 여러 처치가 발생해도 각 처치 연출은 기본 `0.18`초 간격으로 예약되어, 효과음·히트스톱·슬로 모션·화면 효과·처치 텍스트가 정해진 순서로 재생된다. 이 예약은 비스케일 시간 기준이며 일시정지 중에는 진행되지 않는다.
- 피해 판정과 전투 완료 상태는 연출을 기다리지 않고 즉시 처리한다. 전투 완료 시 콤보 규칙 상태만 초기화하며 이미 시작했거나 예약된 마지막 처치 연출은 취소하지 않는다.
- `CombatFeedbackController`가 히트스톱과 슬로 모션의 `Time.timeScale`을 단독 소유하므로 두 시간 연출이 서로의 기준 배율을 덮어쓰지 않는다.
...
월드에 표시되는 `n연속 처치!` 문구의 처치 수는 `PlayerShoot`의 한 번의 발사 시퀀스를 기준으로 한다. HUD의 지속 콤보 수는 별도로 Duel Clock 카운트 제한 안에서 이어진다.

처치 문구와 전반적인 처치 연출 강도는 모두 한 번의 실린더 발사 시퀀스 안에서 발생한 처치 수를 기준으로 한다. 기본값에서는 같은 발사 시퀀스의 처치 순서가 1 증가할 때마다 배율이 `0.2`씩 계속 증가한다. 이 배율은 처치 파티클 수와 크기, 방향 잔상, 화면 플래시, Volume Pulse, 슬로 모션, Fullscreen Impact, 처치 문구에 적용된다. 카메라 셰이크는 접근성 및 화면 안정성을 위해 연속 처치 수와 무관한 고정 처치 세기를 사용한다. 카운트를 넘겨 유지되는 HUD `Combo`는 표시·통계·보너스 골드에만 사용하며 처치 연출 세기에는 관여하지 않는다.

HUD 콤보는 처치 시 8칸으로 갱신된다. 플레이어 행동 자체는 게이지를 소비하지 않으며, DUEL CLOCK이 100%에 도달해 Beat가 커밋되는 즉시 한 칸의 감소를 시작한다. 여러 Beat가 한 번에 커밋되면 각 칸은 `0.1`초 간격으로 순서대로 감소한다. 처치 시 진행 중인 감소 큐를 취소하고 게이지를 다시 8칸으로 갱신한다.

`SFX_Combo_Die`의 피치는 같은 발사 시퀀스의 처치 순서가 증가할 때마다 단조 증가하며, 높은 연속 처치에서도 갑자기 같은 값으로 고정되지 않도록 최대 피치에 점진적으로 접근하는 곡선을 사용한다. 다음 실린더 발사가 시작되면 다시 첫 처치 피치에서 시작한다.
...
- 같은 프레임에 Fullscreen Impact가 4개를 초과해도 활성 슬롯을 덮어쓰지 않는다. 초과 요청은 FIFO 대기열에 보존하고 빈 슬롯이 생기는 순서대로 모두 재생한다.
- 일시정지 시 진행 중인 Fullscreen Impact를 삭제하지 않고 화면에서만 숨긴다. 재개하면 남은 시간부터 이어서 재생한다.
- 적의 활성 자식뿐 아니라 비활성 자식까지 검색해 가장 큰 스프라이트를 캡처한다. 캡처 시점에 유효한 스프라이트가 없더라도 적 위치를 보존하고, 처치 실루엣·잔상은 대체 다이아몬드 형상으로 재생한다.
- 일반·치명타·치명적 피해·처치 시그니처는 큰 프레임 지연이 발생해도 최소 3개의 렌더 프레임을 확보한다.
- 접근성 플래시 배율이 0이거나 `presentationEnabled`가 꺼진 경우는 사용자가 명시적으로 연출을 비활성화한 상태이므로 재생 보장 대상에서 제외한다.
...
| `Assets/Scripts/Common/CombatPresentation.cs` | 총구·기본 명중 연출과 상황별 시그니처 연출 위임 |
| `Assets/Scripts/Common/CombatImpactSignaturePresenter.cs` | 치명타 정밀 잠금, 치명적 피해 압축·지연 파열, 처치 실루엣·분리 잔상, 마지막 적 실린더 봉인 |
...
1. 일반·크리티컬 발사 순간에 총구 섬광과 화면 노출·렌즈 킥이 각각 보이며 명중 헤일로보다 먼저 읽히는지 확인한다.
2. 어두운 적과 배경에서도 일반 공격의 국소 접촉 섬광·이중 스냅 마크와 `Impact Snap`이 분명히 보이고, 크리티컬은 십자선·약실 잠금, 최대 체력 75% 이상 피해는 안쪽 압축·지연 파열, 처치는 실루엣 정지·분리 잔상으로 서로 다른 형태를 유지하는지 확인한다.
3. 마지막 적 처치에서 여섯 약실의 실린더 봉인이 잠긴 뒤 깨지고, 일반 처치보다 실루엣 정지 시간이 길어지는지 확인한다.
4. 크리티컬 처치에서는 크리티컬 대미지 프리팹, 일반 처치에서는 일반 대미지 프리팹이 나오는지 확인한다.
5. 한 번의 연속 발사로 적 두 명 이상을 처치했을 때 `2연속 처치!`, `3연속 처치!`가 순서대로 표시되는지 확인한다.
6. 첫 처치에는 보너스 골드가 없고, 두 번째 처치부터 공식대로 보너스가 발생하는지 확인한다.
7. 마지막 두 적을 같은 발사로 처치했을 때 마지막 텍스트까지 콤보 수에 포함된 뒤 전투 완료가 발생하는지 확인한다.
8. 획득량만큼 골드 이미지가 생성되고, 각 이미지가 Money 패널에 도착할 때 수치가 1씩 증가하는지 확인한다.
9. 골드가 비행 중인 상태로 상점에 진입하면 남은 금액이 즉시 한 번만 반영되는지 확인한다.
10. 먼 거리로 밀어내도 발차기 이동이 `pushFlightDuration` 안에 끝나는지 확인한다.
11. 최대 체력의 25%, 50%, 100% 피해에서 카메라 셰이크 세기와 지속 시간이 선형 비율로 변하는지 확인한다.
12. 허공 발사에서 최대값의 40% 셰이크가 발생하는지 확인한다.
13. 짧은 셰이크와 연속 셰이크 뒤에도 카메라 위치가 점프하지 않고, 회전이 `(0, 0, 0)`으로 부드럽게 돌아오는지 확인한다.
14. 한 프레임에 5개 이상의 타격이 요청되어도 Fullscreen Impact가 순서대로 모두 재생되는지 확인한다.
15. 타격 연출 도중 일시정지했다가 재개해도 Fullscreen Impact와 시그니처 연출이 삭제되지 않고 이어지는지 확인한다.
16. 적 아바타 오브젝트가 비활성화되거나 스프라이트가 비어 있는 순간에도 해당 위치에 처치 실루엣과 잔상이 대체 형상으로 표시되는지 확인한다.
