...
레인은 Y축에서 `LaneDistance` 간격으로 중앙 정렬한다. 가로 배치는 아래의
행 오프셋 규칙을 따른다. 레인을 지정하지 않는 위치 조회는 0번 레인을 사용하며,
새로운 `TryGetTilePosition(tileIndex, laneIndex, out position)` 오버로드가 실제
레인 위치를 제공한다. 상하 이동, 적의 레인 추적, 공격 판정과 스폰 경고는 이
좌표 API를 사용한다.

`WaveManager`는 사용 가능한 모든 `(tile, lane)` 셀 중 현재 생존 적 수가 가장
적은 레인을 우선하고, 동률인 레인과 그 안의 타일은 무작위로 선택한다. 같은
가로 열에 플레이어가 있으면 어느 레인이든 스폰 후보에서 제외해 즉시 포위되는
상황을 피한다. 레거시 저장 필드 `reservedSpawnTileIndices`는 이름과 타입을
유지하지만 값은 `lane * BoardCount + tile` 셀 키로 저장한다. 기존 저장의
`0..BoardCount-1` 값은 자연스럽게 0번 레인으로 복원된다.


## Staggered outline and transparent grid (260924)

- `BoardManager`가 전장 좌표를 소유한다. 위 행은 아래 행보다 한 칸 오른쪽에
  배치되며 전체 외곽의 중심을 기준으로 정렬한다. 예: 위 `01111`, 아래 `11110`.
- `BattleData.BoardCount`는 행당 타일 수이며 전체 타일 수는 계속
  `BoardCount * LaneCount`다. 행당 한 칸뿐인 전장은 연결이 끊기지 않도록
  가로 오프셋을 사용하지 않는다.
- 저장/스폰/경고 셀 ID는 기존 `(tileIndex, laneIndex)`를 유지한다.
  실제 가로 열은 `GetColumnIndex`로 조회한다. 일반 전장의 경우
  `column = tileIndex + laneIndex`다. 저장 스키마, 버전, 키는 변경하지 않았다.
  이전 저장도 같은 행/로컬 타일 번호에서 새 위치로 복원된다.
- 월드 좌표에서 타일 번호를 구할 때는 행을 명시한다. 캐릭터의 높이 오프셋과
  점프 애니메이션 때문에 월드 Y로 행을 추정하지 않는다. 행 없는 기존 조회는
  0번 행에 대한 호환 API다.
- 상하 이동은 실제 X 좌표를 유지한다. 위로 이동할 때 로컬 타일 번호는 하나
  줄고, 아래로 이동할 때 하나 늘어난다. 없는 모서리 칸/점유/예약 칸으로의
  이동은 거부하며 턴을 소비하지 않는다. 성공한 이동 거리는 한 칸이다.
- 적의 전열/추적 거리와 스폰의 플레이어 열 제외는 실제 열 좌표를 사용한다.
  적이 상하로 나갈 수 없는 모서리에 있으면 안쪽으로 한 칸 이동해 추적한다.
- `BoardTile`이 표시를 소유한다. 프리팹 원본은 보존하고 생성된 인스턴스의
  스프라이트 렌더러 대신 사각형 메시를 사용한다. 내부는 3.5% 알파,
  내부선은 28%, 외곽선은 65% 알파다. 공유 경계는 한 번만 그린다.
  경고 중인 칸은 붉은 반투명 면으로 강조되며 기존 경고 소유권을 따른다.
- `Resources/Shaders/BattleGrid.shader`는 URP용 무조명 정점색 셰이더다.
  타일용 이미지나 텍스처를 추가하지 않는다. 메시와 머티리얼은 타일 파괴 시
  해제한다. 카메라 가장자리 이동은 모든 행의 양 끝을 포함한다.

검증: BoardManagerTests의 좌표 왕복, 높이 오프셋, 부모 변환, 모서리 이동,
이동 거리, 점유 거부, 경고 소유권 및 저장 필드 테스트. Unity Play Mode에서
아래 행 (3,0) → 위 행 (2,1) 이동 시 X 유지, Y +0.92, 턴 +1을 확인했다.

검증 결과: 관련 EditMode 테스트 101개 통과. 추가 SceneIntegrityTests는 18개 중 15개 통과, 3개 실패(상점 제목 순서, 보물 UI 참조, 보물 버튼 전환 방식). 해당 장면/프리팹은 이번 작업에서 변경하지 않았다. C# 컴파일 및 신규 셰이더 검사에 오류 없음. 전체 전투 완주와 WebGL 빌드는 수행하지 않았다.
