...
the cylinder has finished resolving. Every progress source stops at the next
`100` boundary and reserves exactly one enemy Beat. The HUD holds the completed
meter at `100%` until that enemy cycle starts. Additional natural time and
player actions do not create hidden progress or another queued cycle while the
reservation is held.
...
meter is followed by two independent status cards: the combined number of
living and unspawned enemies, and the remaining COUNTs until the next
reinforcement, for example `5 COUNT` down to `1 COUNT`. `BattleData` authors
the interval and `DuelClockController` derives the countdown from the already-
saved cumulative Beat count. When the pool is empty, the next-spawn value
changes to gray `완료`. The HUD queries `WaveManager` and never owns or mutates
competing combat state.
...
progress pauses for the same firing interval. During an active enemy cycle,
natural or paid progress may reserve one following cycle. `WaveManager` keeps
