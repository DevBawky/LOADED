...
- 한 번의 준비에서 폭탄을 놓을 수 있는 레인 하나를 Unity 난수로 먼저 선택하고, 그 레인 안에서 중복 없는 타일을 선택한다. 따라서 다중 레인 전장에서는 아래·위 레인이 모두 투척 대상이 될 수 있다. 프로젝트에는 별도 시드 서비스가 없으므로 기존 시스템과 같은 `UnityEngine.Random`을 사용한다.
- 후보가 요청 수보다 적으면 가능한 수만 준비하며 후보가 없으면 빈 공격으로 정상 완료한다.
- 폭탄 투척 준비와 비행 중에는 폭탄 Telegraph를 표시하지 않는다.
- 설치된 폭탄의 퓨즈가 1이 되는 즉시, 같은 레인의 설치 타일 중심 좌우 `bombExplosionRadius`칸을 공통 반투명 붉은 타일 채움으로 표시한다. 전장 밖의 셀은 제외한다.
...
- 산탄 준비 시 유효한 좌우 인접 타일만 붉은 타일 채움으로 표시한다. 위치 변경이 확정되면 기존 공격 의도 갱신 경로에서 새 레인과 인접 타일로 표시를 갱신한다.
...
- `WaveManager`가 적 사이클 종료 단계에서 `BossBombManager.ProcessEnemyTurnCycleEnd`를 호출해 퓨즈를 처리한다. 폭발 판정이 모두 끝난 뒤 `EnemyTurnCycleCompleted`를 보낸다.
- 생성 시 현재 적 턴 라운드 번호를 저장하고 같은 라운드 종료 이벤트는 건너뛴다.
- 이후 라운드 종료마다 1 감소하며 0이면 즉시 폭발한다.
- 런타임 퓨즈는 항상 1~3으로 제한한다.
- 퓨즈 1에서는 폭탄이 점멸한다. 피격 범위 타일은 계속 채워 두어 위험 셀을 읽을 수 있게 한다.
- `BossBomb`이 자기 소유의 타일 경고를 등록하고 폭발 판정 완료 시 잔광으로 해제하고, 비활성화·제거 시 즉시 해제한다. 저장된 퓨즈 복원 시 범위를 다시 표시하며, 다른 공격과 겹친 셀은 마지막 경고가 해제될 때까지 유지한다. 폭탄·산탄의 기존 선 머티리얼은 더 이상 범위 표시에 필요하지 않다.
- 퓨즈가 0이 되면 폭탄 외형을 숨기고 폭발 효과를 먼저 재생한다. 범위 타일은 밝은 테두리로 회피 창 동안 유지하고 판정 완료 후 0.18초 잔광으로 소멸한다. 그 직후부터 `EnemyData.Attack Dodge Window Duration` 동안 플레이어가 폭발 범위를 벗어날 수 있다. 범위를 벗어난 첫 프레임에 회피 성공과 연출을 즉시 확정하고, 기본 0.2초가 끝날 때까지 성공하지 못한 플레이어에게만 피해를 적용한다.
...
폭발은 큐와 중복 방지 집합으로 처리한다. 범위 안의 다른 폭탄은 같은 프레임에 폭발 연출과 회피 창을 시작하고, 각 폭탄이 한 번씩 독립 피해를 적용하므로 겹치는 범위의 피해도 각각 적용된다. 컬렉션 순회에는 스냅샷을 사용한다. `WaveManager`는 모든 폭발 회피 창과 피해 판정이 끝난 뒤에만 해당 적 사이클 완료 이벤트를 보내므로 다음 사이클과 자동 저장이 미결 폭발을 건너뛰지 않는다.

피해 범위는 설치 셀과 같은 레인에서 좌우 `bombExplosionRadius`칸이다. 타일 인덱스가 범위 안이더라도 다른 레인의 플레이어·적·폭탄은 피해와 연쇄 폭발 대상이 아니다. 플레이어와 일반 적은 `bombDamage`, 빅 베럴은 `bossSelfExplosionDamage`를 받는다. 적 보호막은 `EnemyController.ApplyDamageInternal`을 통해 먼저 흡수한다. 현재 PlayerHealth에는 보호막 시스템이 없으므로 플레이어는 기존 `ApplyDamage` 흐름을 사용한다. 폭탄 피해에는 밀어내기나 상태 이상을 적용하지 않는다.
...
3. 준비 후 이동해도 폭탄과 산탄 대상 타일이 바뀌지 않으며, 반복 준비 시 위·아래 레인이 모두 폭탄 대상으로 선택되는지 확인한다.
...
- 빅 베럴도 일반 추적형 적과 함께 좌우 방향별 전열을 판정한다. 해당 방향에서 가장 가까운 추적형 적일 때만 플레이어 레인을 향해 이동하고, 후방이면 현재 레인에서 전진한다.
- 폭탄은 준비 시점에 투척 가능한 레인 중 하나를 선택해 귀속되고, 산탄은 준비 시점의 보스 레인에 귀속된다.
- 폭탄 폭발 범위, 연쇄 폭발, VFX와 산탄 피해는 같은 레인 안에서만 판정한다.
- 플레이어가 다른 레인으로 피하면 해당 폭탄이나 산탄에 피해를 받지 않는다.
- 폭탄 레인과 보스의 준비 공격 레인은 전투 저장/복원에 포함된다.
