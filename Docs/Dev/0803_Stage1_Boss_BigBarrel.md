...
- 퓨즈가 0이 되면 폭탄과 Telegraph를 즉시 숨기고 폭발 효과를 먼저 재생한다. 그 직후부터 `EnemyData.Attack Dodge Window Duration` 동안 플레이어가 폭발 범위를 벗어날 수 있다. 범위를 벗어난 첫 프레임에 회피 성공과 연출을 즉시 확정하고, 기본 0.2초가 끝날 때까지 성공하지 못한 플레이어에게만 피해를 적용한다.
