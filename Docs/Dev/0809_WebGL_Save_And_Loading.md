...
전용 템플릿은 별도 이미지나 영상을 추가하지 않고 CSS로 실린더 장전 진행도와 필름 노이즈를 표현한다. 로딩 중에는 게임 규칙 팁을 순환 표시하고, 준비가 끝나면 실린더 회전 후 `클릭하여 시작` 화면을 연다. 시작 입력은 브라우저 오디오 활성화에도 사용한다. 실행 중에는 음소거와 전체 화면 버튼을 제공하며, 로딩 실패 시 한국어 오류 안내와 다시 시도 버튼을 표시한다.
