...
구매한 아이템을 제한된 인벤토리에 보관하고, 상점·인벤토리·다음 탄환 UI에서 동일한 데이터 기반 툴팁을 화면 밖으로 잘리지 않게 표시할 시스템이 필요했다. 이후 요구사항 변경으로 동일한 아이템도 여러 슬롯에 보유할 수 있게 했다.

### Why AI Was Used

상점 추첨, 제한된 슬롯 인벤토리, 즉시 사용 효과와 UI 갱신을 기존 Manager 구조에 안전하게 연결하기 위해 AI를 사용했다.
...
* 최초 요구사항은 같은 아이템 중첩 불가였으나, 후속 요구사항에 따라 최대 3개 슬롯 안에서 동일 아이템 중복 보유를 허용합니다.
...
`ItemData`, `PlayerInventory`, `InventoryUI`를 추가·확장해 최대 3칸 인벤토리를 구현했다. 동일한 `ItemData`도 빈 슬롯이 있는 동안 여러 개 보유할 수 있다. 아이템 사용 경로에서는 `PlayerMove.CompleteTurn()`을 호출하지 않는다.
...
`ShopManager`는 한 번의 상점 재고 안에서 아이템 후보를 중복 없이 균등 추첨해 두 슬롯에 표시한다. 구매 성공 시 첫 빈 인벤토리 칸에 등록하고 해당 오퍼 버튼의 `interactable`만 끈다. 인벤토리가 가득 찬 경우에만 추가 구매할 수 없으며, 이미 보유한 아이템과 같은 종류도 다시 구매할 수 있다.

260803 상점 재고 갱신 버그 수정 이후 `OpenShop()`은 탄환 및 아이템 버튼 오브젝트와 `interactable`을 먼저 모두 활성화하고 이전 방문의 구매 플래그를 초기화한 뒤 새 재고를 생성한다. 이전 스테이지에서 구매한 슬롯 상태가 다음 상점에 남지 않는다. 동일 아이템 보유 여부는 구매 가능 조건에 사용하지 않으며, 현재 인벤토리에 빈 슬롯만 있으면 같은 아이템을 다시 구매할 수 있다. 한 번의 상점 방문에서 이미 구매한 현재 오퍼만 비활성화되고, 새 상점 방문에서는 새 오퍼 기준으로 다시 활성화된다.
...
* 검증 방법:
  ...
  * 최대 3칸과 동일 `ItemData` 중복 보유 허용 코드 확인
...
* 테스트 결과:
  ...
  * 새 상점은 이전 상점에서 비활성화된 슬롯을 새 오퍼 기준으로 다시 활성화한다.
  * 동일 아이템을 이미 보유해도 빈 인벤토리 슬롯이 있으면 다시 구매할 수 있다.
...
상점에서 같은 확률로 등장하는 일반 아이템을 구매해 최대 3칸 인벤토리에 동일 아이템도 중복 보관하고, 턴 소비 없이 즉시 사용할 수 있게 되었다.
