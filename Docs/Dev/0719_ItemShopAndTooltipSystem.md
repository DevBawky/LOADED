...
260802 상점 재고 갱신 버그 수정 이후 새 상점을 열 때는 이전 상점의 구매 플래그를 먼저 초기화하고, 현재 인벤토리에 추가할 수 있는 아이템만 새 후보로 사용한다. 이전 스테이지에서 구매해 아직 보유 중인 동일 아이템이 다시 추첨되어 비활성 버튼으로 남는 현상을 방지하며, 구매 가능한 남은 아이템은 첫 슬롯부터 다시 배치되어 버튼 상태가 새 재고 기준으로 활성화된다. 현재 두 아이템 중 하나를 보유하고 있다면 나머지 하나만 활성 슬롯으로 표시되고, 인벤토리가 가득 찼거나 모든 후보를 이미 보유했다면 구매 불가능한 빈 슬롯은 숨긴다.
...
* 테스트 결과:
  ...
  * 새 상점은 현재 보유 중인 동일 아이템을 후보에서 제외하고, 이전 상점에서 비활성화된 슬롯을 새 오퍼 기준으로 다시 활성화한다.
