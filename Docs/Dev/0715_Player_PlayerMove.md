...
전장이 두 개 이상의 레인을 사용할 때 `W/S`는 같은 실제 가로 열의 위/아래 레인으로 한 칸 이동하고, `A/D`는 현재 레인에서 좌/우로 한 칸 이동한다. 플레이어 방향 전환은 `Q`, 대기는 `E`, 재장전은 기존처럼 `R`을 사용한다. 레인 이동도 일반 이동과 같이 한 타일 이동 이벤트와 턴 완료를 발생시키며, 현재 레인 인덱스는 런 저장에 포함된다.
