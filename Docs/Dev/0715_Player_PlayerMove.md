...
## 후속 변경: 대기 복원과 우클릭 약실 제거

S 키와 기존 Wait 버튼은 다시 `PlayerMove.Wait`를 호출하며 행동 없이 한 턴을 소비한다. 약실 제거는 실린더에서 원하는 탄환을 우클릭하는 별도 입력으로 분리했다. 선택한 탄환은 파괴되지 않고 사용한 탄환 순환으로 이동하며, 약실 제거 자체는 턴을 소비하지 않는다.

## 후속 변경: 즉시 행동 입력 간격

성공한 대기와 재장전 사이에는 기본 0.16초의 공통 간격을 적용한다. 값은 Player 오브젝트의 `PlayerMove > Action Timing > Instant Action Cooldown`에서 조정한다. 실패하거나 실행 조건을 만족하지 못한 입력은 간격을 소비하지 않는다. 이동, 회전, 사격은 각자의 기존 실행 시간으로 제한되며 이 간격을 적용받지 않는다. 따라서 즉시 행동 직후에도 적 공격에 대한 이동 회피는 바로 사용할 수 있다.

## 후속 변경: 다중 레인 조작

전장이 두 개 이상의 레인을 사용할 때 `W/S`는 같은 타일 인덱스의 위/아래 레인으로 한 칸 이동하고, `A/D`는 현재 레인에서 좌/우로 한 칸 이동한다. 플레이어 방향 전환은 `Q`, 대기는 `E`, 재장전은 기존처럼 `R`을 사용한다. 레인 이동도 일반 이동과 같이 한 타일 이동 이벤트와 턴 완료를 발생시키며, 현재 레인 인덱스는 런 저장에 포함된다.
