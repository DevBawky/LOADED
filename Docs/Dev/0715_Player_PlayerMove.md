...
S 키와 기존 Wait 버튼의 플레이어 행동은 약실 교환으로 대체했다. 입력과 실행 책임은 실린더 행동을 조율하는 `PlayerShoot`으로 이동했으며, 다음 발사 예정 탄환이 있을 때만 그 탄환을 사용한 탄환 순환으로 옮기고 한 턴을 소비한다. 실린더가 비어 있으면 아무 상태도 바꾸지 않고 턴도 소비하지 않는다.
