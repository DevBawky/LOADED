...
전장이 두 개 이상의 레인을 사용할 때 `W/S`는 같은 실제 가로 열의 위/아래 레인으로 한 칸 이동하고, `A/D`는 현재 레인에서 좌/우로 한 칸 이동한다. 플레이어 방향 전환은 `Q`, 대기는 `E`, 재장전은 `R`을 누르는 순간 실행한다. 레인 이동도 일반 이동과 같이 한 타일 이동 이벤트와 턴 완료를 발생시키며, 현재 레인 인덱스는 런 저장에 포함된다.

승인된 레인 이동은 시작 즉시 목적 레인을 게임플레이 위치로 확정하고 시각
이동만 보간한다. 따라서 적 공격의 회피 윈도우가 이동 애니메이션 종료와 같은
프레임에 끝나더라도 상하 이동이 이전 레인에 남아 있는 것으로 판정되지 않는다.

행이 어긋난 전장에서는 상하 이동 시 행 내부 타일 번호가 바뀌지만 실제 X 좌표는 유지한다. 인접 행에 대응하는 칸이 없으면 이동과 턴 소비를 모두 거부한다.
