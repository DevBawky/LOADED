...
`BulletLine`은 전달된 `BulletData`를 `Data`에 보관한다. 후속 Enemy 시스템 연결에서 `PlayerShoot`이 정면 사거리 안의 적을 가까운 순서로 찾고 공격력과 관통 확률을 적용하도록 확장했다. 밀어내기, 기절, 표식 실행은 아직 구현하지 않았다. 이동하는 Projectile은 제거했으며 발사선은 `lineDuration` 동안 표시된 뒤 자동 제거된다.

`WaveManager.GetEnemiesInDirection`은 플레이어 타일을 기준으로 바라보는 방향과 `MaxRange` 안의 적을 가까운 순서로 제공한다. 첫 적은 확정 적중하고 이후 적은 `Penetration Chances`를 단계별로 판정한다. LineRenderer의 시작점은 `firePoint.position`, 끝점은 마지막 적중 적의 위치다. 유효한 적이 없으면 발사는 실패하며 탄환과 턴을 소비하지 않는다.
...
발사 시 Fire Point에서 관통 판정을 통과한 마지막 적까지 LineRenderer가 즉시 표시된다. 적중한 적은 `BulletData.Damage`만큼 체력이 감소하며, 유효한 적이 없으면 장전 탄환과 턴을 소비하지 않는다.
